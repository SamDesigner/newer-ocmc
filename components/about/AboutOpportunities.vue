<template>
  <div
    class="max-w-[1440px] mx-auto px-[20px] lg:px-[64px] py-[64px] md:py-[112px] text-coolGray"
  >
    <section class="flex flex-col gap-[40px] lg:gap-[80px] md:flex-row">
      <div data-aos="fade-right" data-aos-duration="1000" class="flex-1">
        <h1
          class="text-[40px] font-[400] telegraf text-coolGray leading-[48px]"
        >
          Providing Exceptional Opportunities for Business Sectors
        </h1>
      </div>
      <div
        data-aos="fade-left"
        data-aos-duration="1000"
        class="flex-1 flex flex-col gap-[32px]"
      >
        <p class="text-[18px] font-[400] leading-[27px] text-collerGray">
          At OCMC, we specialise in delivering a comprehensive range of bid
          writing and compliance solutions to healthcare and social care
          companies, prioritising cost reduction, efficiency improvement, and
          gaining a competitive edge.
        </p>
        <div class="py-[8px] flex flex-col md:flex-row gap-[24px]">
          <div
            data-aos="fade-right"
            data-aos-duration="1000"
            class="flex flex-1 flex-col gap-[16px]"
          >
            <h3
              class="telegraf text-[20px] font-[400] leading-[28px] text-coolGray"
            >
              Why Choose Us?
            </h3>
            <p class="text-[16px] font-[400] leading-[24px] text-coolerGray">
              We offer Increasing cost and operational efficiency, flexible
              services and a distinct competitive advantage to name a few 
            </p>
          </div>
          <div
            data-aos="fade-left"
            data-aos-duration="1000"
            class="flex flex-1 flex-col gap-[16px]"
          >
            <h3
              class="telegraf text-[20px] font-[400] leading-[28px] text-coolGray"
            >
              Our Approach
            </h3>
            <p class="text-[16px] font-[400] leading-[24px] text-coolerGray">
              We provide independent contract and business solutions to meet the
              unique needs of our clients.  
            </p>
          </div>
        </div>
      </div>
    </section>

    <section class="md:my-20" data-aos="fade-up">
      <div class="grid grid-cols-2 items-center">
        <div class="space-y-5">
          <h3 class="telegraf md:text-5xl text-3xl">
            Pursuing Success Through Exceptional Service
          </h3>
          <p class="leading-relaxed">
            At OCMC, completed a wide range of contract procurement projects and
            achieved significant success. Our expertise in tender procurement
            allows us to deliver exceptional contract outcomes, helping our
            clients boost their operational efficiency, and gain competitive
            advantage at an affordable rate.
          </p>
        </div>

        <div class="">
          <img src="/img/pursuing-grid-img.png" alt="" class="w-3/4 mx-auto" />
        </div>
      </div>
    </section>

    <section class="md:max-w-3xl mx-auto">
      <div class="grid md:grid-cols-2 md:gap-20 gap-10">
        <div class="space-y-7">
          <div class="space-y-3">
            <img src="/svg/competitive-advantage.svg" alt="" />
            <h3 class="telegraf text-lg">Competitive Advantage</h3>
          </div>
          <p>
             We provide top-tier services that provides you with a competitive
            advantage ensuring your company  stands out in the market
          </p>
        </div>
        <div class="space-y-7">
          <div class="space-y-3">
            <img src="/svg/regulatory-assurance.svg" alt="" />
            <h3 class="telegraf text-lg">Regulatory Assurance</h3>
          </div>
          <p>
             Rely on our compliance expertise for seamless adherence to
            regulations, leaving you worry free.
          </p>
        </div>
      </div>
    </section>
  </div>
</template>
